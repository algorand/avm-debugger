    ...
    static parseMethodResponse(method, methodResult, pendingInfo) {
        ...
        try {
            ...
            if (method.returns.type !== 'void') {
                ...
                if (logs.length === 0) {
                    throw new Error(`App call transaction did not log a return value ${JSON.stringify(pendingInfo)}`);
                }
                const lastLog = logs[logs.length - 1];
                if (lastLog.byteLength < 4 ||
                    !(0, utils_1.arrayEqual)(lastLog.slice(0, 4), RETURN_PREFIX)) {
                    throw new Error(`App call transaction did not log a ABI return value ${JSON.stringify(pendingInfo)}`);
                }
