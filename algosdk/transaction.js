...
function getKeyregKey(input, inputName, length) {
    ...
    if (typeof input === 'string') {
        inputAsBuffer = (0, binarydata_1.base64ToBytes)(input);
    }
    else if (input.constructor === Uint8Array) {
        inputAsBuffer = input;
    }
    if (inputAsBuffer == null || inputAsBuffer.byteLength !== length) {
        throw Error(`${inputName} must be a ${length} byte Uint8Array or base64 string.`);
    }
...
class Transaction {
    constructor({ ...transaction }) {
        ...
        this.tag = new TextEncoder().encode('TX');
        // Populate defaults
        ...
        txn.sender = address.decodeAddress(txn.sender);
        if (txn.receiver !== undefined)
            txn.receiver = address.decodeAddress(txn.receiver);
        ...
        if (txn.assetSender !== undefined)
            txn.assetSender = address.decodeAddress(txn.assetSender);
        ...
        txn.genesisHash = (0, binarydata_1.base64ToBytes)(txn.genesisHash);
        ...
        if (txn.assetMetadataHash !== undefined &&
            ...
            if (typeof txn.assetMetadataHash === 'string') {
                txn.assetMetadataHash = new Uint8Array(new TextEncoder().encode(txn.assetMetadataHash));
            }
    ...
    get_obj_for_encoding() {
        if (this.type === 'pay') {
            ...
            if (this.receiver !== undefined)
                txn.rcv = this.receiver.publicKey;
            if (!txn.note.length)
                delete txn.note;
            if (!txn.amt)
                delete txn.amt;
        ...
        if (this.type === 'axfer') {
            ...
            if (this.assetSender !== undefined)
                txn.asnd = this.assetSender.publicKey;
            // allowed zero values
            if (!txn.note.length)
                delete txn.note;
            if (!txn.lx.length)
        ...
        if (this.type === 'appl') {
            ...
            if (this.appAccounts !== undefined) {
                txn.apat = this.appAccounts.map((decodedAddress) => decodedAddress.publicKey);
            }
            // allowed zero values
            if (!txn.note.length)
                delete txn.note;
    ...
    static from_obj_for_encoding(txnForEnc) {
        ...
        txn.sender = address.decodeAddress(address.encodeAddress(new Uint8Array(txnForEnc.snd)));
        if (txnForEnc.grp !== undefined)
            txn.group = txnForEnc.grp;
        ...
        if (txnForEnc.type === 'pay') {
            ...
            txn.receiver = address.decodeAddress(address.encodeAddress(new Uint8Array(txnForEnc.rcv)));
            ...
            if (txnForEnc.asnd !== undefined) {
                txn.assetSender = address.decodeAddress(address.encodeAddress(new Uint8Array(txnForEnc.asnd)));
            }
            txn.receiver = address.decodeAddress(address.encodeAddress(new Uint8Array(txnForEnc.arcv)));
        }
    ...
    _getDictForDisplay() {
        ...
        forPrinting.sender = address.encodeAddress(forPrinting.sender.publicKey);
        if (forPrinting.receiver !== undefined)
            forPrinting.receiver = address.encodeAddress(forPrinting.receiver.publicKey);
        ...
        if (forPrinting.assetSender !== undefined)
            forPrinting.assetSender = address.encodeAddress(forPrinting.assetSender.publicKey);
        ...
        if (typeof forPrinting.genesisHash !== 'string')
            forPrinting.genesisHash = (0, binarydata_1.bytesToBase64)(forPrinting.genesisHash);
        return forPrinting;
    }
