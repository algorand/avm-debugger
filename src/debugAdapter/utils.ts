...
export class TxnGroupSourceDescriptorList {
    ...
    static async loadFromFile(fileAccessor: FileAccessor, txnGroupSourcesDescriptionPath: string): Promise<TxnGroupSourceDescriptorList> {
        const rawGroupSourcesDescription = Buffer.from(await fileAccessor.readFile(txnGroupSourcesDescriptionPath));
        const jsonGroupSourcesDescription = JSON.parse(rawGroupSourcesDescription.toString('utf-8')) as Record<string, any>;
        const txnGroupSources = (jsonGroupSourcesDescription['txn-group-sources'] as any[]).map(source => TxnGroupSourceDescriptor.fromJSONObj(fileAccessor, txnGroupSourcesDescriptionPath, source));
...
export class TEALDebuggingAssets {
    ...
    constructor(simulateResponse: algosdk.modelsv2.SimulateResponse, txnGroupDescriptorList: TxnGroupSourceDescriptorList) {
    ...
    static async loadFromFiles(fileAccessor: FileAccessor, simulateResponsePath: string, txnGroupSourcesDescriptionPath: string): Promise<TEALDebuggingAssets> {
        const rawSimulateResponse = Buffer.from(await fileAccessor.readFile(simulateResponsePath));
        const simulateResponse = algosdk.modelsv2.SimulateResponse.from_obj_for_encoding(
            parseJsonWithBigints(rawSimulateResponse.toString('utf-8'))
        );

        const txnGroupDescriptorList = await TxnGroupSourceDescriptorList.loadFromFile(
            fileAccessor,
